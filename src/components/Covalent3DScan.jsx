...
//     const sketch = (p) => {
//       p.setup = () => {
//         p.createCanvas(1100, 600, p.WEBGL).parent('tokenText');
//       };
...
//     try {
//       const response = await fetch(
//         `https://api.covalenthq.com/v1/1/address/${address}/balances_v2/?key=${import.meta.env.VITE_COVALENT_API_KEY}`
//       );
//       const json = await response.json();
//       setTokensData(json.data.items);
//     } catch (error) {
...
//         </div>
//       </div>
//       <div id="tokenText" className="token-info-container" ref={canvasRef}></div>
//       <div className="token-container">
//         {tokensData.map((token, index) => (
//           <div
//             key={index}
//             className={`token ${selectedTokenIndex === index ? 'selected' : ''}`}
//             onClick={() => handleTokenClick(index)}
//             style={{ backgroundColor: `rgb(${getColorByToken(token.contract_name).join(',')})` }}
//           >
//             <h2>{token.contract_name}</h2>
//             <h2>{token.quote}</h2>
//            <img src={token.logo_url} alt="Token Logo" className="token-logo" style={{ width: '64px', height: '64px' }} />
//           </div>
//         ))}
//       </div>
//     </>
